package io.github.forgestove.bottle_ship.config.tree;
import com.google.common.collect.ImmutableList;
import com.mojang.datafixers.util.Pair;
import io.github.forgestove.bottle_ship.config.annotation.*;
import io.github.forgestove.bottle_ship.config.annotation.Range;
...
public final class RootConfigNode<C> implements ConfigNode<C> {
	public static final Component TITLE = Component.translatable("bottle_ship.config.title");
	...
	private static class Builder<C> {
		...
		private CategoryConfigNode<C> createCategoryNode(Field categoryField) {
			...
			var categoryBuilder = CategoryConfigNode.<C>builder()
				.name(categoryName)
				.title(Component.translatable("bottle_ship.config.category." + snakeCase(categoryName)));
		...
		private <T> void addSingleValueField(
			...
			var titleKey = "bottle_ship.config.option." + snakeCase(categoryField.getName()) + "." + snakeCase(valueName);
		...
		private <T> ValueConfigNode.ValueValidator<T> makeRangeValidator(int min, int max) {
			return value -> {
				if (value instanceof Integer intVal) {
					if (intVal < min) return Component.translatable("bottle_ship.config.validator.min", min);
					if (intVal > max) return Component.translatable("bottle_ship.config.validator.max", max);
				}
				return null;
			};
		}
